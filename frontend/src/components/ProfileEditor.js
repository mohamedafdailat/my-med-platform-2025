...
const ProfileEditor = ({ user, language }) => {
  ...
  const semesterLocked = validSemester(user.semester);
  ...
  return (
    <form onSubmit={save} className="space-y-4 border-b border-gray-200 pb-6 mb-6" aria-label={ar ? 'المعلومات الشخصية' : 'Informations personnelles'}>
      <p className="text-sm text-gray-600">{ar ? 'يمكنك تعديل بيانات الاتصال. بعد اختيار الفصل الدراسي، لا يمكن تغييره إلا بواسطة المسؤول.' : 'Modifiez vos coordonnées. Une fois votre semestre choisi, seul l’administrateur peut le changer.'}</p>
      ...
      <label className="block text-sm font-semibold text-gray-700" htmlFor="profile-semester">
        ...
        <select id="profile-semester" name="semester" value={fields.semester} onChange={change} required disabled={saving || semesterLocked} className={inputClass}>
      ...
      <p className="text-xs text-gray-500">{semesterLocked ? (ar ? 'هذا هو الفصل الدراسي الذي يحدد محتوى مكتبتك. لتغييره، اتصل بالمسؤول.' : 'Ce semestre détermine le contenu de votre bibliothèque. Pour le changer, contactez l’administrateur.') : (ar ? 'اختر فصلك الدراسي مرة واحدة لفتح محتواه.' : 'Choisissez votre semestre une seule fois pour accéder au contenu correspondant.')}</p>
